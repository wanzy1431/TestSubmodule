...
public final class ThreadPoolManager {
     ...
     * 向线程池中添加任务方法
     */
    public void addExecuteTask(Runnable task, boolean isBase) {
        if (isBase)
            addExecuteTask(task, 5);
        else
            addExecuteTask(task, 4);
    }
     ...
     * 向线程池中添加任务方法
     */
    public void addExecuteTask(Runnable task, int number) {
        if (task != null) {
            String threadName = null;
            if (Thread.currentThread().getStackTrace().length > number) {
                threadName = getThreadName(Thread.currentThread().getStackTrace()[number].getClassName(),
                        Thread.currentThread().getStackTrace()[number].getMethodName());
            }
            mThreadPool.execute(new CustomRunnable(task, threadName));
        }
    }

    @NonNull
    private String getThreadName(String clazz, String method) {
        return clazz + DOT + method + THREAD_NAME_SUFFIX;
    }
    ...
    public static class CustomRunnable implements Runnable {
        ...
        public CustomRunnable(Runnable runnable, String name) {
