package com.unis.common_module.widget.swipe;
...
public class SwipeItemLayout extends FrameLayout {
    ...
    public SwipeItemLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(R.styleable.SwipeItemLayout);
...
//                || (!isClickable() && mMenus.size() > 0);
    ...
    private class DragCallBack extends ViewDragHelper.Callback {
        ...
        public void onViewReleased(@NonNull View releasedChild, float xvel, float yvel) {
            LogUtil.e(TAG + "：onViewReleased: " + xvel + " ,releasedChild = " + releasedChild);
            if (isLeftMenu()) {
                if (xvel > mVelocity) {
                    open();
